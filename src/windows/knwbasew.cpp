...
KnowledgeBaseWnd::KnowledgeBaseWnd(struct KnowledgeBase* pKb, WINDOW* screen) {
    gKb = pKb;
    hScreen = screen;

    #ifdef _MSVC
        sprintf_s(id, "msgBoxWnd");
        sprintf_s(hTitle, 120, "%s", pKb->getTitle());
    #else
...
void KnowledgeBaseWnd::readKnowledgeBase() {
    ...
    for (int i = 0; i < categoriesSize; i++) {
        ...
        #ifdef _MSVC
            sprintf_s(item->title, 80, "%s", gKbCategories[i]->title);
        #else
            sprintf(item->title, "%s", gKbCategories[i]->title);
        #endif
        item->subItems = (ListItem**)malloc(200 * sizeof(ListItem));
        ...
        for (int i2 = 0; i2 < gKb->getArticlesCount(i); i2++) {
            ...
            #ifdef _MSVC
                sprintf_s(item->subItems[i2]->title, 100, "%s", articles[i2]->title);
            #else
